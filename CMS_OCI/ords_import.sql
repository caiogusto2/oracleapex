...
-- Schema: WKSP_DATAPREV  Date: Tue Feb 18 04:25:54 2025 
--
...
BEGIN
  ORDS.ENABLE_SCHEMA(
      p_enabled             => TRUE,
      p_schema              => 'WKSP_DATAPREV',
      p_url_mapping_type    => 'BASE_PATH',
      p_url_mapping_pattern => 'dataprev',
      p_auto_rest_auth      => FALSE);
    
  ORDS.DEFINE_MODULE(
      p_module_name    => 'indenderecos',
      p_base_path      => '/indenderecos/',
      p_items_per_page => 25,
      p_status         => 'PUBLISHED',
      p_comments       => NULL);
  ...
  ORDS.DEFINE_TEMPLATE(
      p_module_name    => 'indenderecos',
      p_pattern        => 'recebimento/:bucket_name',
      p_priority       => 0,
      p_etag_type      => 'HASH',
      p_etag_query     => NULL,
      p_comments       => NULL);
...
FROM 
    dbms_cloud.list_objects(''CREDENCIALDATAPREV'',''https://objectstorage.sa-vinhedo-1.oraclecloud.com/n/idi1o0a010nx/b/'' || :bucket_name || ''/o/'') c, controle_upload l
...
order by l.data_upload');
  ...
  ORDS.DEFINE_PARAMETER(
      ...
      p_method             => 'GET',
      p_name               => 'bucket_name',
      p_bind_variable_name => 'bucket_name',
  ...
  ORDS.DEFINE_TEMPLATE(
      p_module_name    => 'indenderecos',
      p_pattern        => 'recebimento/:bucket_name/:file_name',
      p_priority       => 0,
      p_etag_type      => 'HASH',
      p_etag_query     => NULL,
      p_comments       => NULL);
...
'begin
    dbms_cloud.delete_object(
        credential_name => ''CREDENCIALDATAPREV'',
        object_uri      => ''https://objectstorage.sa-vinhedo-1.oraclecloud.com/n/idi1o0a010nx/b/'' || :bucket_name || ''/o/'' || :file_name || '''');
...
BEGIN
   ...
   v_object_uri := ''https://objectstorage.sa-vinhedo-1.oraclecloud.com/n/idi1o0a010nx/b/demo-dataprev/o/'' || v_uniqname;
   v_extensao := :extensao;
   ...
   INSERT INTO APICALLAUX (
       ...
       v_tempid,   -- Ensure this matches the column data type
       v_uniqname, 
       v_file_name,
       v_extensao,
       v_tempdate,
   ...
   -- Validation checks
   IF v_bucket_name IS NULL 
      OR v_file_name IS NULL 
      OR v_column_count <> 12
      OR v_row_count = 0
      OR v_header_mismatch > 0
   THEN
   sys.htp.p(sys.htf.escape_sc(''Erro ao processar arquivo: Corrija os pro' || 'blemas com o template e tente novamente'')); 
   sys.htp.p(sys.htf.escape_sc(''O arquivo precisa ter 12 colunas, possui: '' || v_column_count));
...
');
  ...
  ORDS.CREATE_ROLE(p_role_name => 'INDENDERECOS');
  ORDS.CREATE_ROLE(p_role_name => 'oracle.dbtools.role.autorest.WKSP_DATAPREV');
    
  l_roles(1) := 'INDENDERECOS';
  l_modules(1) := 'indenderecos';

  ORDS.DEFINE_PRIVILEGE(
      p_privilege_name => 'INDENDERECOS',
      p_roles          => l_roles,
      p_patterns       => l_patterns,
      p_modules        => l_modules,
      p_label          => 'INDENDERECOS',
      p_description    => NULL,
      p_comments       => NULL); 
  ...
  l_roles(2) := 'oracle.dbtools.role.autorest.WKSP_DATAPREV';

  ORDS.DEFINE_PRIVILEGE(
      p_privilege_name => 'oracle.dbtools.autorest.privilege.WKSP_DATAPREV',
      p_roles          => l_roles,
      p_patterns       => l_patterns,
      p_modules        => l_modules,
      p_label          => 'WKSP_DATAPREV metadata-catalog access',
      p_description    => 'Provides access to the metadata catalog of the objects in the WKSP_DATAPREV schema.',
      p_comments       => NULL); 
